...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', version: '0.1.3', uptime: Math.floor(process.uptime()) });
});
...
if (existsSync(publicDir)) {
  ...
  app.use((req, res) => res.json({ message: 'TavernShelf API v0.1.3 — frontend not built' }));
}
...
async function start() {
  logger.info('Boot', 'TavernShelf v0.1.3 starting');
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`[TavernShelf] v0.1.3 listening on :${PORT}`);
